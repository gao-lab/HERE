...
single.contig.Alu.only.vcf.gz.filename <- paste(sep="", snakemake@params[["result_directory"]], "/", snakemake@wildcards[["CHROMOSOME"]], ".Alu.only.vcf.gz" )
if (FALSE) {
    single.contig.Alu.only.vcf.gz.filename <- "result/sA02_5__generate_all_transcribable_Alu_A_vcf/hg38.fa/chr22.Alu.only.vcf.gz"
}
