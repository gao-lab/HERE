...
input.gencode.3utr.and.all.edit.CJ.dt.csv.gz.filename <- snakemake@input[["gencode_3utr_and_all_edit_CJ_dt_csv_gz_filename"]]
...
output.all.edited.gencode.3utr.maf.table.with.species.id.for.a.single.chromosome.filename <- snakemake@output[["all_edited_gencode_3utr_maf_table_with_species_id_for_a_single_chromosome_filename"]]

gencode.3utr.and.all.edit.CJ.dt <- fread(input.gencode.3utr.and.all.edit.CJ.dt.csv.gz.filename)
...
gencode.3utr.and.all.edit.edited.block.only.with.human.alignment.dt <- merge(
    x=gencode.3utr.and.all.edit.CJ.dt[edit.is.in.UTR.block == TRUE],
    y=raw.3UTR.alignment.dt[id==9606],
    by.x=c("transcript_id"),
    by.y=c("transcript.id"),
    all.x=FALSE, all.y=FALSE)

gencode.3utr.and.all.edit.edited.block.only.with.human.alignment.dt[, edited.alignment:=lapply(alignment, FUN=function(temp.alignment){
...
if (nrow(gencode.3utr.and.all.edit.edited.block.only.with.human.alignment.dt) == 0) {
    warning("No 3'-UTR edited in this file: ", input.gencode.3utr.maf.table.with.species.id.for.a.single.chromosome.filename)
    writeLines("NA", output.all.edited.gencode.3utr.maf.table.with.species.id.for.a.single.chromosome.filename)
    q(save="no")
}
...
gencode.3utr.and.all.edit.edited.block.only.with.human.alignment.dt[, transcript_and_edit_info:=paste(sep="", transcript_id, "@", edit.POS, "@", edit.rel.POS.wrt.3UTR)]

all.edited.3UTR.alignment.dt <- foreach(temp.row.dt=iter(gencode.3utr.and.all.edit.edited.block.only.with.human.alignment.dt, by="row")) %do% {
...
fwrite(all.edited.3UTR.alignment.dt, output.all.edited.gencode.3utr.maf.table.with.species.id.for.a.single.chromosome.filename, sep="\t", col.names=FALSE, quote=FALSE)
