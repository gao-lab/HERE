...
if (@ARGV != 4) {
	die "need to provide 1 inputs: shared variant file and outputfile Alu/non-Alu name\n";
}
my ($Alu,$inputfile, $outputfile1,$outputfile2) = @ARGV;
...
close o1;
############### Step 2 : bedtools ###################
my $command = "$bedtools -a $inputfile.temp -b $Alu -wa > $outputfile1.temp";
system ("$command");
my %hash;
...
$command = "$bedtools -a $inputfile.temp -b $Alu -v > $outputfile2.temp";
system ("$command");
my %hash2;
