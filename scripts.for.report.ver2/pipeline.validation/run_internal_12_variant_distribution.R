...

{
    ...
    .[, event.summary.collapsed.ordered:=factor(
            ...
            levels=c(
                "A>G", "T>C", "A>G;T>C",
                "C>T", "G>A", "C>T;G>A",
                "A>C", "T>G", "A>C;T>G", "A>T", "T>A", "A>T;T>A",
                "C>A", "G>T", "C>A;G>T", "C>G", "G>C", "C>G;G>C",
                "mixed variants"
            ))] -> .;
    ##
    . -> ..to.plot.dt
    ##
    ## start plotting
    ggplot(..to.plot.dt, aes(x=event.summary.collapsed.ordered, y=log10(count+0.1), fill=positive.type)) ->.;
    ## plot boxplots
    . + geom_boxplot() ->.;
    ## add zero line
    ...
    . + labs(x="validation result", y="log10(# variants + 0.1) per sample", fill="") ->.;
    ## save image
    ggsave.A4(
        filename=glue("{output.directory}/validation.all.variants.png"),
        plot=.,
