...
all.edited.intersection.of.TargetScan.and.miRanda.compared.with.original.annotated.summary.gene.and.edit.level.dt <- fread(glue("{output.directory}/all.edited.intersection.of.TargetScan.and.miRanda.compared.with.original.annotated.summary.gene.and.edit.level.dt.gz"))
...

{
    ...
    merge(x=., y=all.edited.intersection.of.TargetScan.and.miRanda.compared.with.original.annotated.summary.gene.and.edit.level.dt,
    ...
    .[, value.clipped:=cut(value, breaks=c(-Inf, 0, 1, 2, Inf))] -> .;
    .[, value.clipped.reformatted:=factor(c("(-Inf,0]"="0", "(0,1]"="1", "(1,2]"="2", "(2,Inf)"=">=3")[value.clipped], levels=c("0", "1", "2", ">=3"))] -> .;
    ##= prettify stages
    ...
    ##
    ## compute the pvalue
    temp.pvalue <- dcast(temp.data.dt, Gene_ID+SAMPLE~metric.renamed, value.var="value")[, wilcox.test(x=`MBS gain`, y=`MBS lost`, alternative="greater", paired=TRUE)]$"p.value"
    if (temp.pvalue == 0){
        temp.pvalue <- "p < 2.22e-16"
    }

    fwrite(temp.data.dt, glue("{output.directory}/source.data.for.REE.MBS.gain.and.loss.count.percentage.barplot.png.csv.gz"))
    
    ##
    ## run ggplot
    ggplot(copy(temp.data.dt)[, total.count:=.N, metric.renamed][, list(percentage=(.N/total.count)*100), list(metric.renamed, value.clipped.reformatted, total.count)], aes(x=metric.renamed, y=percentage, fill=value.clipped.reformatted)) -> .;
    . + geom_bar(stat='identity', position="stack") ->.;
    . + stat_pvalue_manual(data=data.table(metric.renamed="MBS gain", value.clipped.reformatted="0", count=1.3, group1="MBS gain", group2="MBS lost", p.value=temp.pvalue), label="p.value", y.position=110, vjust=-0.3) -> .;
    ##. + stat_compare_means(data=data.table(temp.data.dt, count=0), mapping=aes(y=value), method="wilcox.test", paired=TRUE, comparisons=list(c("MBS gain", "MBS lost")), method.args=list(alternative="greater")) -> .;
    . + labs(x="effect of REE on MBS", y="%", fill="# REEs per\n(gene,\nsample)") -> .;
    . + theme_pubr() ->.;
    . + guides(fill=guide_legend(nrow=2)) -> .;
    . + scale_y_continuous(limits=c(0,120), breaks=seq(0, 100, 20)) ->.;
    . + scale_fill_manual(values=c("grey50", "#DD5532", "#FF6F39", "#FFA338")) ->.;
    ggsave.A4(
        filename=glue("{output.directory}/REE.MBS.gain.and.loss.count.percentage.barplot.png"),
        plot=.,
...

{
    ...
    . + labs(x="effect of REE on MBS", y="# REEs per (gene, sample)\n(upper-bounded by 10)") -> .;
    . + theme_pubr() ->.;
    . + scale_y_continuous(limits=c(0,12), breaks=seq(0, 10, 2)) ->.;
    ggsave.A4(
        filename=glue("{output.directory}/REE.MBS.gain.and.loss.count.boxplot.png"),
        plot=.,
...
## plot per gene x sample the counts of MBS-gaining RE between MTC and other maternal genes
{
    ...
    ## copy a data for the statistical test
    temp.data.dt <- copy(.)
    fwrite(temp.data.dt, glue("{output.directory}/source.data.for.REE.MBS.gain.count.on.MTC.or.nonMTC.maternal.genes.barplot.png.csv.gz"))
    ...
    ggsave.A4(
        filename=glue("{output.directory}/REE.MBS.gain.count.on.MTC.or.nonMTC.maternal.genes.barplot.png"),
        plot=.,
...
#### the filled bar version
{
    ...
    ggsave.A4(
        filename=glue("{output.directory}/REE.MBS.net.change.count.on.MTC.or.nonMTC.maternal.genes.barplot.png"),
        plot=.,
