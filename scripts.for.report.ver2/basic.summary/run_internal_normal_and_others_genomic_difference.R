...
## per-stage count, barplot
{
    ...
    .[is.na(Annotation.simplified) == TRUE, Annotation.simplified:="splicing/CDS"] -> .;
    ...
    .[, source:=c("TRUE"="normal only", "FALSE"="abnormal only", "TRUE;FALSE"="both", "FALSE;TRUE"="both")[source.raw]] -> .;
    ## get counts
    .[, list(count=.N), list(Annotation.simplified, stage, source)] -> .;
    ## prettify stage
    ...
    .[, Annotation.simplified.ordered:=factor(Annotation.simplified, levels=c("3'-UTR", "intron", "exon (ncRNA)", "5'-UTR", "splicing/CDS", "intergenic/intragenic"))] -> .;
    . -> ..to.plot.dt
    ##
    ## plot 
