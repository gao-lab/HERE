...
output.directory <- "./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/all.normal.samples/RE.and.expression"
...
## 7. Plot scatterplots for AF.median ~ FPKM.median under different conditions
{
    ...
    ## 7.1. plot all gene x edit pairs
    {
        ...
        . + geom_text(data=unique(..to.plot.dt[, list(stage.pair.ordered, spearman.cor.test.pvalue.adjusted.prettified, spearman.cor.test.pvalue.adjusted, spearman.cor, AF.median.in.stage.previous=0.5, FPKM.median.in.stage.current=2e-3)]), mapping=aes(label=glue("Spearman's rho:\n{round(spearman.cor, 4)}\nBH-adjusted p-value:\n{spearman.cor.test.pvalue.adjusted.prettified}")), size=3) -> .;
        ...
        ggsave.A4(filename=glue("{output.directory}/AF.median.vs.FPKM.median.scatterplot.overall.png"), plot=., width.r=0.9, height.r=0.5)
    ...
    ## 7.3. plot all gene x edit pairs, targets of maternal clearance or not, MBS annotation type
    {
        ...
        . + labs(x="median(editing level) in the previous stage\n(>=0.1 due to identification pipeline)", y="median(FPKM) in the current stage") -> .;
        ggsave.A4(filename=glue("{output.directory}/AF.median.vs.FPKM.median.scatterplot.wrt.cluster.and.MBS.annotation.png"), plot=., width.r=0.9, height.r=1)
    }
...
## 10. Plot scatterplots for diff.AF.median ~ diff.FPKM.median under different conditions
{
    ...
    ## 10.1. plot histogram of transition types, focusing on targets of maternal clearance only, focusing on cleared genes only
    {
        ...
        .[diff.FPKM.median < 0] -> .;
        .[, stage.pair.ordered:=factor(stage.pair.named.vector[paste(sep="", stage.previous, "->", stage.current)], levels=stage.pair.named.vector)] -> .;
        .[, transition.type.prettified:=c("RE.to.RE"="is still a RE in the latter stage", "RE.to.non-RE"="is not a RE in the latter stage")[transition.type]] -> .;
        .[, cluster.prettified:=c("maternal.decay"="decay at 8-cell", "maternal.others"="others")[cluster]] -> .;
        . -> ..to.plot.dt;
        ##
        ...
        . + labs(x="", y="# (gene, RE) pairs\nwhose median gene FPKM drops\nupon stage transition", fill="RE group") -> .;
        ggsave.A4(filename=glue("{output.directory}/transition.type.histogram.gene.median.FPKM.drop.only.wrt.cluster.png"), plot=., width.r=0.75, height.r=0.3)
        ##
    } -> not.used.variable
