...
output.directory <- "./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/GSE95477.RE.matching.edits.and.expression.and.MBS/"
...
## 3.6.BACKUP.
{
    ...
    {
        fwrite(..GSE95477.RE.matched.edits.with.MBS.annotation.count.on.maternal.gene.only.dt, "./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/GSE95477.RE.matched.edits.with.MBS.annotation.count.on.maternal.gene.only.dt.gz")
    }
...
## RETURN
{
    ...
    ## 4.2. add RE-matched edit counts per gene to oocyte GV samples
    {
        merge(
            ...
            y=GSE95477.RE.matched.edits.with.MBS.annotation.count.on.maternal.gene.only.dt[, list(
                SAMPLE.oocyte.GV=SAMPLE, maternal.bin.ordered,
                Gene_ID, Gene_Name, cluster,
                gains.miRNA.sites,
                RE.matched.edits.count.oocyte.GV=RE.matched.edits.count)],
            by="SAMPLE.oocyte.GV",
            all.x=TRUE, all.y=FALSE) -> .;
    } -> ..GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.dt;

    ## 4.3. add expression per gene to oocyte MII samples
    {
        merge(
            x=..GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.dt,
          ...
          all.x=TRUE, all.y=FALSE) -> .;
    } -> ..GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.and.with.oocyte.MII.FPKM.dt
    
    ## 4.3.BACKUP. write current result to file
    {
        fwrite(..GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.and.with.oocyte.MII.FPKM.dt, "./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.and.with.oocyte.MII.FPKM.dt.gz")
    }
...
## GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.and.with.oocyte.MII.FPKM.dt <- fread("./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/GSE95477.patient.info.no.outliers.with.oocyte.GV.RE.matched.edit.counts.per.MBS.annotation.and.with.oocyte.MII.FPKM.dt.gz")
{
    ...
    ## 5.1. summarize for each gene the spearman cor of RE-matched edit count (oocyte GV) ~ FPKM (oocyte MII)
    {
        ...
        ), list(Gene_ID, Gene_Name, cluster, gains.miRNA.sites)] -> .;
    ...
    ## 5.2. plot distribution of spearman correlation coefficients (histogram)
    {
        ...
        . + geom_histogram() -> .;
        . + theme_pubr() -> .;
        . + facet_grid(gains.miRNA.sites~., scales="free_y") -> .;
...

{
    ...
    ## 6.2. plot distribution of spearman correlation coefficients (histogram)
    {
        ...
        . + geom_histogram() -> .;
        . + theme_pubr() -> .;
        . + facet_grid(gains.miRNA.sites~maternal.bin.ordered, scales="free_y") -> .;
        . + labs(x="Spearman correlation coefficient", y="# Genes", fill="") ->.;       
    ...
    ## 6.3. plot distribution of spearman correlation coefficients for each cluster separately (density plot)
    {
        ...
        . + geom_density(alpha=0.5) -> .;
        . + theme_pubr() -> .;
        . + facet_grid(cluster.renamed.ordered~gains.miRNA.sites) -> .;
        . + labs(x="Spearman correlation coefficient", y="density", fill="") ->.;
    ...
    ## 6.4. plot distribution of spearman correlation coefficients for each cluster separately (boxplot)
    {
        ...
        . + theme_pubr() -> .;
        . + theme(axis.text.x=element_blank()) -> .;
        . + facet_grid(gains.miRNA.sites~cluster.renamed.ordered) -> .;
        . + labs(x="", y="Spearman correlation coefficient", fill="") ->.;
        . + scale_y_continuous(limits=c(-1.0, 1.3)) -> .;
        ggsave.A4(filename=glue("{output.directory}/spearman.cor.for.each.cluster.boxplot.YAO.png"), plot=., width.r=0.45, height.r=0.4)
    }
...
## RETURN
{
    ...
    {
        merge(
            x=GSE95477.RE.matched.edits.with.MBS.annotation.count.on.maternal.gene.only.dt[, list(SAMPLE, stage, maternal.bin.ordered, Gene_ID, Gene_Name, cluster, RE.matched.edits.count, gains.miRNA.sites)],
            y=combined.gexpr.FPKM.melt.GSE95477.samples.only.dt[, list(SAMPLE=SAMPLE, Gene_ID, FPKM=FPKM)],
            by=c("Gene_ID", "SAMPLE"),
            all.x=FALSE, all.y=FALSE) -> .;
    } -> ..GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.dt;

    ## 7.2. filter against outlier samples

    {
        ..GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.dt[(SAMPLE %in% c("GSM2514781","GSM2514773")) == FALSE] -> .;
        .
    } -> ..GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.no.outlier.samples.dt;
    
    ## 7.2.BACKUP. write current result to file
    {
        fwrite(..GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.no.outlier.samples.dt, "./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.no.outlier.samples.dt.gz")
    }
...
## GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.no.outlier.samples.dt <- fread("./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/GSE95477.RE.matched.edits.count.per.MBS.annotation.and.FPKM.on.maternal.gene.only.no.outlier.samples.dt.gz")
{
    ...
    ## 8.2. plot distribution of spearman correlation coefficients (histogram) within same sample
    {
        ...
        . + geom_histogram() -> .;
        . + theme_pubr() -> .;
        . + facet_grid(gains.miRNA.sites~stage, scales="free_y") ->.;
        . + labs(x="Spearman correlation coefficient", y="# Genes", fill="") ->.;       
        ggsave.A4(filename=glue("{output.directory}/spearman.cor.histogram.within.sample.png"), plot=., width.r=0.6, height.r=0.3)
    }
    ...
    ## 8.4. plot distribution of spearman correlation coefficients for each cluster separately (boxplot)
    {
        ...
        . + scale_y_continuous(limits=c(-1.0, 1.3)) -> .;
        . + theme(axis.text.x=element_blank()) -> .;
        ggsave.A4(filename=glue("{output.directory}/spearman.cor.for.each.cluster.boxplot.within.sample.png"), plot=., width.r=0.45, height.r=0.45)
    }
...

{
    ...
    ## 9.2. plot distribution of spearman correlation coefficients (histogram)
    {
        ...
        . + geom_histogram() -> .;
        . + theme_pubr() -> .;
        . + facet_grid(gains.miRNA.sites~stage+maternal.bin.ordered, scales="free_y") -> .;
        . + labs(x="Spearman correlation coefficient", y="# Genes", fill="") ->.;       
    ...
    ## 9.3. plot distribution of spearman correlation coefficients for each cluster separately (density plot)
    {
        ...
        . + geom_density(alpha=0.5) -> .;
        . + theme_pubr() -> .;
        . + facet_grid(cluster.renamed.ordered~stage + gains.miRNA.sites) -> .;
        . + labs(x="Spearman correlation coefficient", y="density", fill="") ->.;
        ggsave.A4(filename=glue("{output.directory}/spearman.cor.for.each.cluster.density.within.sample.YAO.png"), plot=., width.r=0.9, height.r=0.3)
    }
    ...
    ## 9.4. plot distribution of spearman correlation coefficients for each cluster separately (boxplot)
    {
        ...
        ggsave.A4(filename=glue("{output.directory}/spearman.cor.for.each.cluster.boxplot.within.sample.YAO.png"), plot=., width.r=0.9, height.r=0.3)
    }
