...
edited.intersection.of.TargetScan.and.miRanda.compared.with.original.annotated.summary.gene.and.edit.level.dt <- fread(glue("{output.directory}/edited.intersection.of.TargetScan.and.miRanda.compared.with.original.annotated.summary.gene.and.edit.level.dt.gz"))
...

{
    ...
    merge(x=., y=edited.intersection.of.TargetScan.and.miRanda.compared.with.original.annotated.summary.gene.and.edit.level.dt,
    ...
    . + labs(x="effect of RE on MBS", y="%", fill="# REs per\n(gene,\nsample)") -> .;
    . + theme_pubr() ->.;
    . + guides(fill=guide_legend(nrow=2)) -> .;
    . + scale_y_continuous(limits=c(0,120), breaks=seq(0, 100, 20)) ->.;
    . + scale_fill_manual(values=c("grey50", "#DD5532", "#FF6F39", "#FFA338")) ->.;
    ggsave.A4(
        filename=glue("{output.directory}/RE.MBS.gain.and.loss.count.percentage.barplot.png"),
        plot=.,
...

{
    ...
    ggsave.A4(
        filename=glue("{output.directory}/RE.MBS.gain.and.loss.count.boxplot.png"),
        plot=.,
...
## plot per gene x sample the counts of MBS-gaining RE between MTC and other maternal genes
{
    ...
    ggsave.A4(
        filename=glue("{output.directory}/RE.MBS.gain.count.on.MTC.or.nonMTC.maternal.genes.barplot.png"),
        plot=.,
...
#### the filled bar version
{
    ...
    ggsave.A4(
        filename=glue("{output.directory}/RE.MBS.net.change.count.on.MTC.or.nonMTC.maternal.genes.barplot.png"),
        plot=.,
